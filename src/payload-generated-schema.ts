...
export const artwork = sqliteTable(
  'artwork',
  {
    ...
    camera: text('camera'),
    lens: text('lens'),
    settings_aperture: text('settings_aperture'),
    settings_shutter: text('settings_shutter'),
    settings_iso: numeric('settings_iso'),
  ...
  (columns) => ({
    artwork_image_idx: index('artwork_image_idx').on(columns.image),
    artwork_slug_idx: index('artwork_slug_idx').on(columns.slug),
    artwork_updated_at_idx: index('artwork_updated_at_idx').on(columns.updatedAt),
    artwork_created_at_idx: index('artwork_created_at_idx').on(columns.createdAt),
    artwork__status_idx: index('artwork__status_idx').on(columns._status),
  }),
)

export const _artwork_v = sqliteTable(
  '_artwork_v',
  {
    id: integer('id').primaryKey(),
    parent: integer('parent_id').references(() => artwork.id, {
      onDelete: 'set null',
    }),
    ...
    version_series: text('version_series'),
    version_year: numeric('version_year'),
    version_description: text('version_description'),
    version_medium: text('version_medium'),
    version_dimensions: text('version_dimensions'),
    version_location: text('version_location'),
    version_camera: text('version_camera'),
    version_lens: text('version_lens'),
    version_settings_aperture: text('version_settings_aperture'),
    version_settings_shutter: text('version_settings_shutter'),
    version_settings_iso: numeric('version_settings_iso'),
  ...
  (columns) => ({
    _artwork_v_parent_idx: index('_artwork_v_parent_idx').on(columns.parent),
    _artwork_v_version_version_image_idx: index('_artwork_v_version_version_image_idx').on(
      columns.version_image,
    ),
    _artwork_v_version_version_slug_idx: index('_artwork_v_version_version_slug_idx').on(
      columns.version_slug,
    ),
    _artwork_v_version_version_updated_at_idx: index(
      '_artwork_v_version_version_updated_at_idx',
    ).on(columns.version_updatedAt),
    _artwork_v_version_version_created_at_idx: index(
      '_artwork_v_version_version_created_at_idx',
    ).on(columns.version_createdAt),
    _artwork_v_version_version__status_idx: index('_artwork_v_version_version__status_idx').on(
      columns.version__status,
    ),
    _artwork_v_created_at_idx: index('_artwork_v_created_at_idx').on(columns.createdAt),
    _artwork_v_updated_at_idx: index('_artwork_v_updated_at_idx').on(columns.updatedAt),
    _artwork_v_latest_idx: index('_artwork_v_latest_idx').on(columns.latest),
    _artwork_v_autosave_idx: index('_artwork_v_autosave_idx').on(columns.autosave),
  }),
)
...
export const products = sqliteTable(
  'products',
  {
    ...
    description: text('description'),
    size: text('size', { enum: ['8x10', '11x14', '16x20', '20x24', '24x30', '30x40'] }).notNull(),
    material: text('material', { enum: ['archival-matte', 'glossy', 'canvas', 'metal'] }).notNull(),
    edition_total: numeric('edition_total').default('0'),
    edition_current: numeric('edition_current').default('0'),
  ...
  (columns) => ({
    products_artwork_idx: index('products_artwork_idx').on(columns.artwork),
...
export const orders = sqliteTable(
  'orders',
  {
    ...
    shipping_address1: text('shipping_address1').notNull(),
    shipping_address2: text('shipping_address2'),
    shipping_city: text('shipping_city').notNull(),
    shipping_state: text('shipping_state').notNull(),
    shipping_zipCode: text('shipping_zip_code').notNull(),
...
export const bookings = sqliteTable(
  'bookings',
  {
    ...
    eventType: text('event_type', {
      enum: ['wedding', 'engagement', 'portrait', 'family', 'corporate', 'event', 'other'],
    }).notNull(),
    eventDate: text('event_date').default(sql`(strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`),
    eventTime: text('event_time'),
    location: text('location'),
    guestCount: numeric('guest_count'),
    ...
    budget: text('budget', {
      enum: ['under-500', '500-1000', '1000-2500', '2500-5000', '5000-10000', 'over-10000'],
    }),
  ...
  (columns) => ({
    bookings_updated_at_idx: index('bookings_updated_at_idx').on(columns.updatedAt),
    bookings_created_at_idx: index('bookings_created_at_idx').on(columns.createdAt),
  }),
)
...
export const payload_locked_documents_rels = sqliteTable(
  ...
  (columns) => ({
    ...
    payload_locked_documents_rels_artwork_id_idx: index(
      'payload_locked_documents_rels_artwork_id_idx',
    ).on(columns.artworkID),
    ...
    artworkIdFk: foreignKey({
      columns: [columns['artworkID']],
      foreignColumns: [artwork.id],
      name: 'payload_locked_documents_rels_artwork_fk',
    }).onDelete('cascade'),
...
export const relations_artwork = relations(artwork, ({ one }) => ({
  image: one(media, {
    fields: [artwork.image],
    references: [media.id],
    relationName: 'image',
  }),
}))
export const relations__artwork_v = relations(_artwork_v, ({ one }) => ({
  parent: one(artwork, {
    fields: [_artwork_v.parent],
    references: [artwork.id],
    relationName: 'parent',
  }),
  version_image: one(media, {
    fields: [_artwork_v.version_image],
    references: [media.id],
...
export const relations_products = relations(products, ({ one }) => ({
  artwork: one(artwork, {
    fields: [products.artwork],
    references: [artwork.id],
    relationName: 'artwork',
  }),
}))
...
export const relations_bookings = relations(bookings, () => ({}))
...
export const relations_payload_locked_documents_rels = relations(
  ...
  ({ one }) => ({
    ...
    artworkID: one(artwork, {
      fields: [payload_locked_documents_rels.artworkID],
      references: [artwork.id],
      relationName: 'artwork',
    }),
...
type DatabaseSchema = {
  ...
  relations_artwork: typeof relations_artwork
  relations__artwork_v: typeof relations__artwork_v
