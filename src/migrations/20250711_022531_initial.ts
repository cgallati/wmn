...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`artwork\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`artwork_image_idx\` ON \`artwork\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`artwork_slug_idx\` ON \`artwork\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`artwork_updated_at_idx\` ON \`artwork\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`artwork_created_at_idx\` ON \`artwork\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`artwork__status_idx\` ON \`artwork\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_artwork_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_artwork_v_parent_idx\` ON \`_artwork_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_version_version_image_idx\` ON \`_artwork_v\` (\`version_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_version_version_slug_idx\` ON \`_artwork_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_version_version_updated_at_idx\` ON \`_artwork_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_version_version_created_at_idx\` ON \`_artwork_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_version_version__status_idx\` ON \`_artwork_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_created_at_idx\` ON \`_artwork_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_updated_at_idx\` ON \`_artwork_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_latest_idx\` ON \`_artwork_v\` (\`latest\`);`)
  await db.run(sql`CREATE INDEX \`_artwork_v_autosave_idx\` ON \`_artwork_v\` (\`autosave\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_artwork_idx\` ON \`products\` (\`artwork_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`artwork\`;`)
  await db.run(sql`DROP TABLE \`_artwork_v\`;`)
